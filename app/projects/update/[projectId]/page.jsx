...
const Update = ({ params }) => {
  ...
  useEffect(() => {
    const handleDefaultData = async () => {
      ...
      setFrameworks(docSnap.data().frameworks);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = fetch(`/api/v1/projects/${params.projectId}`, {
        method: "PUT",
        headers: {
  ...
  const handleFrameInputKeyPress = (event) => {
    if (
      event.key === "Enter" ||
      (event.key === " " && frameInputValue.trim() !== "")
    ) {
      event.preventDefault();
    ...
    <>
      <div className="w-full">
        <ActionBar
          title={"Projects"}
          actions={{
            Cancel: `/projects/${params.projectId}`,
          }}
        />
      </div>
      <div className="w-full md:max-w-[65rem] p-2 mt-10 ">
        <div className="text-4xl my-4 w-full">Updating existing project...</div>
        <MarkdownHelp />
        ...
        <div className="flex justify-start items-center w-full mt-2">
          <input
            ...
            inputMode="text"
            className="rounded border border-zinc-600 mx-2 px-2 py-0 bg-black text-white my-2 focus:outline-none"
          />
        ...
        <div className="border border-zinc-800 bg-zinc-950 w-full rounded-md ">
          <menu className="flex space-x-2 p-1">
            <button
              className={`bg-zinc-800 hover:bg-zinc-900 px-2 py-0.5 w-20 text-center  border-black rounded-sm cursor-pointer ${!showPreview ? "bg-zinc-900" : ""}`}
              onClick={() => setShowPreview(false)}
            >
              Write
            </button>
            <button
              className={`bg-zinc-800 hover:bg-zinc-900 px-2 py-0.5 w-20 text-center rounded-sm cursor-pointer ${showPreview ? "bg-zinc-900" : ""}`}
          ...
          {!showPreview ? (
            <textarea
              ...
              className="h-[40rem] md:min-h-[50rem] bg-neutral-900 border border-zinc-800 w-full p-2 focus:outline-none"
            />
          ) : (
