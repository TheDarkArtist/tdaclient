...
const Sidenav = () => {
  ...
  return (
    <>
      <button
        className="fixed z-30 right-2 top-3 md:hidden"
      ...
      <div
        onClick={toggleSidebar}
        className={`${showSidebar ? "w-full" : "w-0"} z-20 bg-transparent opacity-60 h-full fixed transition-all ease-in-out`}
      />
      <menu
        className={`transform right-0 ${
          showSidebar ? "w-[80%] translate-x-0" : "translate-x-full"
        } fixed z-20 max-w-80 flex h-full flex-col justify-between border-l dark:border-cyan-950 border-stone-600 transition-all ease-in-out bg-stone-400 text-white dark:bg-[#310101] md:hidden dark:backdrop-filter bg-opacity-80 backdrop-blur-sm dark:bg-opacity-40 dark:backdrop-blur-sm shadow-2xl dark:shadow-2xl shadow-stone-800 dark:shadow-red-600`}
      >
